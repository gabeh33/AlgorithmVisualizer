...
# Makes the starting window which prompts the user for input
...
# Draws the board based on the dimensions given in the starting window
def draw_board():
...
# Gets the row and column of the node clicked based on the position of the click and size of the squares
...
# Checks if the node given is within the bounds of the board, its not a wall, and its not in the worklist or visited
...
# Get the distance using basic trig from the given node to the ending node
...
def gen_random_walls():
    for row in range(len(squares)):
        for col in range(len(squares[0])):
            rec, isWall = squares[row][col]
            if isWall:
                # This is already a wall, so dont do anything
                continue
            elif row == startingRow and col == startingCol:
                continue
            elif row == endingRow and col == endingCol:
                continue
            else:
                # Now we decide to make this a wall or not, about 30% should be
                rand = random.randrange(1, 10, 1)
                if rand < 5:
                    # Make it a wall
                    squares[row][col] = (rec, True)
                    rec.setFill('orange')
                    rec.draw(win)
...
if __name__ == '__main__':
    ...
    message.setSize(16)
    message.setText("Place walls manually, or press r to randomly generate walls \n"
                    "Then press the green node to start")
    while True:
        ...
        if point:
            ...
            if not (row == endingRow and col == endingCol) and not isWall:
                ...
                square.draw(win)
    if alg_sel == 'dfs':
        message.setText("Searching using Depth First Search")
        message.setSize(23)
        path = dfs_bfs(True)
    elif alg_sel == 'bfs':
        message.setText("Searching using Breadth First Search")
        message.setSize(23)
        path = dfs_bfs(False)
    elif alg_sel == 'astar':
        message.setText("Searching using A*")
        message.setSize(23)
        path = astar()
